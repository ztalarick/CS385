...
Node* create_bst_helper(int data[], int low, int high, int level) {
	...
	Node *result = new Node(level, data[mid]);
	result->left = create_bst_helper(data, 0, mid - 1, level + 1);
	result->right = create_bst_helper(data, mid + 1, high, level + 1);
	return result;
}
...
bool is_bst(Node *n, int *min, int *max) {
	...
	return is_bst(n->right, &n->data, max) && is_bst(n->left, min, &n->data);
}
